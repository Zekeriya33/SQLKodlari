...
GROUP BY genelde , AVG(), COUNT(), MAX(), MIN() ve SUM() gibi aggregate fonksiyanlari ile 
birlikte kullanilir
*/
...
select ulke,cinsiyet,  count(*)as calisan_sayisi,round(avg(yas)) as yas_ortalamasi,avg(maas) from personel
where maas>30000
group by ulke,cinsiyet
